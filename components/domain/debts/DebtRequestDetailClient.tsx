...
export default function DebtRequestDetailClient({ requestId, currentUserId }: { requestId: string; currentUserId: string }) {
  ...
  return <div className="page-container debt-page">
    ...
    <PageHeader eyebrow="Historical private record" title="Debt request" description="Private to both participants" actions={<DebtStatusBadge status={request.status} />} />
    ...
    {request.status === "pending" && isRequester && <div className="card debt-actions"><h2>Pending confirmation</h2><p className="text-secondary">This request has no balance effect yet.</p><button className="btn btn-danger" disabled={busy} onClick={() => setConfirmCancel(true)}>Cancel request</button></div>}
    <ConfirmDialog open={confirmCancel} title="Cancel this pending debt request?" description="The request will remain in participant history and will have no balance effect." confirmLabel="Cancel request" tone="danger" busy={busy} onCancel={() => setConfirmCancel(false)} onConfirm={() => { setConfirmCancel(false); void mutate(`/api/debts/requests/${request.id}/cancel`); }} />
  </div>;
}
